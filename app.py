...
if st.sidebar.button("Run Live Crisis Analytics Pipeline"):
    with st.spinner("Processing global shipping telemetry layers..."):
        ...
        with col2:
            ...
            m_col1.metric("Disrupted Molecule", context['molecule'], f"Source: {context['blocked_source']}")
            m_col2.metric("Volumetric Impact", f"{context['impacted_volume']:,} Units")
            m_col3.metric("Alternative Routing Chosen", context['recommended_vendor'], f"Price Index: ${context['index_price']}/kg")

            st.markdown("---")
            st.markdown("### 🤖 Agentic Action Blueprint & Governance")
            
            st.info(f"🛡️ **CDSCO Regulatory Status:** {context['cdsco_status']} | **Cryptographic Verification Hash:** `{crypto_hash[:32]}`")
